...
@Transactional
public class CustServiceTest extends AbstractTest {
	...
	@Test
	public void driverCustTest() throws ParseException{
		...
		for (int i = 0; i < testingData.length; i++){
			...
			templateCust(((String) testingData[i][0]), (String) testingData[i][1],(String)testingData[i][2], (Integer) testingData[i][3],(java.util.Date) testingData[i][4], (Boolean) testingData[i][5],(String)  testingData[i][6], (Class<?>) testingData[i][7]);
			this.rollbackTransaction();
		}
	}

	protected void templateCust(String username, String title,String description, Integer gauge, java.util.Date testingData,Boolean isDraft,String newspaperId, Class<?> expected) {
		Class<?> caught;
		caught = null;
		Cust res,saved;
		...
		try{
			...
			res.setGauge(gauge);
			saved = this.custService.save(res);
			this.custService.flush();
			saved.setTitle("Changing the title");
			saved.setIsDraft(false);
			this.custService.save(saved);
			this.custService.flush();
			unauthenticate();
